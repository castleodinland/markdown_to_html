...
def main():
    ...
    print(f"v12.17 启动: Markdown 直出 HTML [在线资源离线嵌入版 + CSP 安全防护增强 + 违规可视化] (共找到 {len(md_files)} 个 MD 文件)\n")
    try:
        import pygments
    except ImportError:
...
'''
            ...
            # 回读文件，用 BS4 解析，取 body.outerHTML 计算哈希（与浏览器完全一致）
            with open(output_file, 'r', encoding='utf-8') as f:
                written_html = f.read()
            re_soup = BeautifulSoup(written_html, 'html.parser')
            body_outer = re_soup.body.encode(formatter='html5').decode('utf-8')
            file_hash = hashlib.sha256(body_outer.encode('utf-8')).hexdigest()

            print(f"[OK] 成功生成: {output_file}")
            print(f"[INTEGRITY] SHA256: {file_hash}")

            # 使用 Playwright 无头浏览器加载 HTML，提取浏览器端完整性校验结果
            try:
