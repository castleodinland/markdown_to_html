...
# v12.16 更新日志:
# 1. 新增 CSP 违规可视化：浏览器端实时监听 securitypolicyviolation 事件
# 2. 违规信息以醒目红色横幅 + 详细列表形式直接显示在页面顶部
# 3. 无需打开 F12 开发者工具，用户即可看到所有被拦截的安全威胁
...
def process_file(md_file_path):
    ...
    style_tag.string = BASE_CSS + LAYOUT_CSS + CODE_THEMES.get("dark", "")
    ...
    function __cspShowViolations() {{
        ...
        var thead = '<thead><tr style="background:#161b22;">'
            + '<th style="padding:8px 12px;text-align:left;border-bottom:2px solid #da3633;color:#f85149;font-weight:700;">#</th>'
            + '<th style="padding:8px 12px;text-align:left;border-bottom:2px solid #da3633;color:#f85149;font-weight:700;">\\u8FDD\\u89C4\\u6307\\u4EE4</th>'
            + '<th style="padding:8px 12px;text-align:left;border-bottom:2px solid #da3633;color:#f85149;font-weight:700;">\\u88AB\\u62E6\\u622A URI</th>'
            + '<th style="padding:8px 12px;text-align:left;border-bottom:2px solid #da3633;color:#f85149;font-weight:700;">\\u4F4D\\u7F6E</th>'
            + '<th style="padding:8px 12px;text-align:left;border-bottom:2px solid #da3633;color:#f85149;font-weight:700;">\\u65F6\\u95F4</th>'
            + '</tr></thead>';
        var tbody = '<tbody>';
        for (var i = 0; i < __cspViolations.length; i++) {{
            ...
            tbody += '<tr style="background:' + rowBg + ';">'
                + '<td style="padding:6px 12px;border-bottom:1px solid #21262d;color:#8b949e;">' + (i+1) + '</td>'
                + '<td style="padding:6px 12px;border-bottom:1px solid #21262d;"><code style="background:rgba(218,54,51,0.15);color:#f85149;padding:2px 6px;border-radius:4px;font-size:12px;">' + v.violatedDirective + '</code></td>'
                + '<td style="padding:6px 12px;border-bottom:1px solid #21262d;word-break:break-all;max-width:300px;">' + v.blockedURI + '</td>'
                + '<td style="padding:6px 12px;border-bottom:1px solid #21262d;color:#8b949e;font-size:12px;">' + loc + '</td>'
                + '<td style="padding:6px 12px;border-bottom:1px solid #21262d;color:#8b949e;font-size:12px;white-space:nowrap;">' + v.timestamp + '</td>'
                + '</tr>';
        }}
        tbody += '</tbody>';
    ...
    # v12.15: CSP 安全策略 - 为内嵌脚本计算 SHA256 哈希并绑定
    script_hash = hashlib.sha256(js_code.encode('utf-8')).digest()
    script_hash_b64 = base64.b64encode(script_hash).decode('utf-8')
    csp_value = (
        f"default-src 'none'; "
        f"script-src 'sha256-{script_hash_b64}'; "
        f"style-src 'unsafe-inline'; "
        f"img-src data: blob:; "
        f"media-src data: blob:; "
        f"font-src data:; "
        f"connect-src 'none'; "
        f"frame-src 'none'; "
        f"object-src 'none'; "
...
def main():
    ...
    print(f"v12.16 启动: Markdown 直出 HTML [在线资源离线嵌入版 + CSP 安全防护 + 违规可视化] (共找到 {len(md_files)} 个 MD 文件)\n")
    try:
        import pygments
    except ImportError:
